...
- **백신 프로그램의 오탐(예: 안랩 V3) — 설치 파일이 격리/삭제됨**: 미서명 설치 파일이 자동 업데이트 기능(Velopack)까지 포함하고 있어, 일부 백신이 이를 의심스러운 패턴으로 오인해 검사 없이 격리하는 경우가 있습니다. 실제 악성코드가 아니며, 다음과 같이 대응할 수 있습니다.
  - V3의 "격리소"(또는 "치료 결과") 화면에서 해당 파일을 **복원 후 예외 처리(검사 제외)** 해주면 다시 실행할 수 있습니다.
  - 설치형(`Setup.exe`) 대신 자동 업데이트 기능이 없는 **Portable(zip)** 버전을 받으면 오탐 확률이 낮아집니다(위 [다운로드](#다운로드) 표 참고).
  - 안랩 홈페이지에서 "오탐 신고"(false positive report) 메뉴를 통해 해당 파일을 신고하면, 검토 후 반영되어 이후 배포판부터는 정상 인식됩니다.
  - 근본적인 해결은 유료 코드서명 인증서 적용이며, 현재는 비용 대비 우선순위상 보류 중입니다.
